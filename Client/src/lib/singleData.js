export const singleData = {
    id: 1017,
    title: "Luxury 4 BHK Villa in ECR",
    img: [
        "https://images.pexels.com/photos/323705/pexels-photo-323705.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        "https://images.pexels.com/photos/1918291/pexels-photo-1918291.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        "https://images.pexels.com/photos/681331/pexels-photo-681331.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        "https://images.pexels.com/photos/6908367/pexels-photo-6908367.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
    ],
    bedrooms: 4,
    bathrooms: 3,
    price: 300000,
    address: "East Coast Road",
    size: 654,
    city: "Chennai",
    school: "50m Away",
    bus: "30m away",
    restaurant: "40m Away",
    description:
        "A charming single-family home featuring a spacious living area, modern kitchen, and cozy bedrooms filled with natural light. The property includes a private backyard, perfect for outdoor gatherings and relaxation. Located in a peaceful neighborhood close to schools, parks, and shopping amenities.",
    latitude: 12.9249,
    longitude: 80.2427,
};
